...
from framework.torch_resnet import ResNet50, ResNet10, ResNet18, ResNet18BN
from framework.mnist_net import MnistNet, CMnistNet
from framework.mlp import mlp, linear
...
from dataset.cmnist import ColoredMNIST
...
class CIFAR10WithScores(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        img, target = self.cifar10_dataset[index]
...
def get_config():
    config = {
            ...
            'num_workers_mnist': 1,
            'num_workers_cmnist': 1,
            'num_workers_svhn': 2,
            'num_workers_cifar10': 4,
            'num_workers_cifar10_m': 4,
            'num_workers_cifar10_admm': 4,
            'num_workers_cifar10_own': 4,
            'num_workers_madry_robust': 2,
...
def get_arch(arch, num_classes, channel, im_size, width=64):
    if arch[0:3] == 'mlp':
        print("MLP with width %d"%width)
        return mlp(int(arch[3:]), width)
    if arch == 'linear':
        return linear()
    if arch == 'resnet20':
    ...
    if arch == 'resnet18BN':
        return ResNet18BN(channel=channel, num_classes=num_classes)
    # channel=channel, num_classes=num_classes
    if arch == 'resnet10':
        return ResNet10(channel=channel, num_classes=num_classes)
    if arch == 'mnistnet':
        return MnistNet()
    if arch == 'cmnistnet':
        return CMnistNet()
...
def get_dataset(dataset, root, transform_train, transform_test, zca=False):
    ...
    if dataset == 'cifar10':
        ...
        trainset = ColoredMNIST(root=root, env='train1', transform=transform_train)
        trainset_test = ColoredMNIST(root=root, env='train2', transform=transform_train)
        testset = ColoredMNIST(root=root, env='test', transform=transform_test)
        num_classes = 2
        shape = [3, 28, 28]
    elif dataset == 'svhn':
        trainset = SVHN(
...
def get_transform(dataset):
    ...
    if dataset == 'cifar10':
    ...
    elif dataset == 'tiny-imagenet-200-rescale':
        default_transform_train = transforms.Compose([
                #transforms.RandomCrop(32, padding=4),
                #transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
        ...
        print('the dataset is tiny-imagenet-200-rescale')
    elif dataset == 'cub-200':
        default_transform_train = transforms.Compose([
                #transforms.RandomCrop(32, padding=4),
                #transforms.RandomHorizontalFlip(),
    ...
    elif dataset == 'mnist' or dataset == 'cmnist':
        default_transform_train = transforms.Compose([
                transforms.ToTensor(),
        ])
        default_transform_test = transforms.Compose([
